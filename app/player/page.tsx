      ...
      {sorted.map((p, i) => (
        ...
        >
          ...
          <span
            style={{
              flex: 1,
              fontWeight: p.id === currentPlayerId ? 700 : 500,
            }}
          >
            {p.name}{" "}
...
export default function PlayerPage() {
  ...
  useEffect(() => {
    if (currentQuestion && gameState?.status === "voting") {
      ...
      const seed =
        currentQuestion.id.charCodeAt(0) + currentQuestion.id.charCodeAt(1);
      const arr = [0, 1, 2];
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data }) => {
        if (data) {
          setMyVote(data as Vote);
          setSelectedVote(data.selected_index);
        } else {
          setMyVote(null);
        }
      });
  ...
  useEffect(() => {
    if (gameState?.status === "revealed" && currentQuestion) {
      supabase
        ...
        .eq("question_id", currentQuestion.id)
        .then(({ data }) => {
          if (data) setAllVotes(data as Vote[]);
        });
  ...
  // ---------- Submit question by default ----------
  useEffect(() => {
    if (myVote) return;
    if (gameState?.status === "voting" && countdown === 0) {
      handleVote();
    }
  }, [myVote, gameState?.status, countdown]);
  ...
  const handleVote = async () => {
    ...
    if (currentQuestion.player_id === playerId) return; // creator can't vote
    setVoteLoading(true);
    setVoteError("");
  ...
  // Was my vote correct?
  const iVotedCorrectly =
    myVote !== null && myVote.selected_index === lieDisplayIndex;
  ...
  const renderWaiting = () => (
    <div style={{ textAlign: "center", padding: "48px 24px" }}>
      ...
      <p style={{ color: "var(--text-secondary)", fontSize: "15px" }}>
        The admin will start the next round soon. Make sure you&apos;ve
    ...
    <div className="animate-fade-in-up">
        ...
        <div>
          ...
          >
            ...
            <strong style={{ color: "var(--text-primary)" }}>
              {(currentQuestion as Question & { players?: { name: string } })
      ...
      >
        {shuffledStatements.map((stmt, displayIdx) => {
          const isCreator = currentQuestion?.player_id === playerId;
          return (
            <button
              key={displayIdx}
              className={`vote-option ${selectedVote === displayIdx ? "selected" : ""}`}
              onClick={() =>
                !myVote && !isCreator && setSelectedVote(displayIdx)
              }
              disabled={!!myVote || isCreator || countdown === 0}
            >
              <span
                style={{
                  display: "flex",
      ...
      {currentQuestion?.player_id === playerId ? (
        <div
          style={{
        ...
        >
          🎭 You created this question! You can&apos;t vote.
        </div>
      ) : !myVote ? (
        <button
  ...
  const renderRevealed = () => {
    const correctVoterIds = allVotes
      .filter((v) => v.selected_index === lieDisplayIndex)
      .map((v) => v.player_id);
    const myPlayer = players.find((p) => p.id === playerId);

    const otherPlayers = players.filter(
      (p) => p.id !== currentQuestion?.player_id,
    );
    const winners = otherPlayers.filter((p) => correctVoterIds.includes(p.id));
    const missedCount = otherPlayers.length - winners.length;
      ...
      <div className="animate-bounce-in">
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div
                    style={{
                      fontSize: "12px",
                      marginTop: "4px",
                      color: isLie ? "#34d399" : "var(--text-muted)",
                    }}
                  >
                    {isLie ? "← This is the LIE" : "Truth"}
                    {iVoted && (
                      <span style={{ marginLeft: "8px" }}>← your vote</span>
                    )}
                  </div>
                </div>
                <span style={{ fontSize: "13px", color: "var(--text-muted)" }}>
                  {
                    allVotes.filter((v) => v.selected_index === displayIdx)
                      .length
                  }{" "}
                  vote(s)
                </span>
          ...
          {currentQuestion?.player_id === playerId ? (
            <>
              <div style={{ fontSize: "28px", marginBottom: "6px" }}>
                {allVotes.length > 0 && winners.length === 0 ? "🔥" : "🎭"}
              </div>
              <div
                style={{
                  fontWeight: 700,
              ...
              >
                {allVotes.length > 0 && winners.length === 0
                  ? "Nobody found your lie. Great job!"
                  : winners.length > 0
                    ? `${winners.length} player(s) found your lie.`
                    : "Waiting for results..."}
              </div>
            </>
          ) : iVotedCorrectly ? (
            <>
              ...
              <div
                style={{ fontWeight: 700, fontSize: "17px", color: "#34d399" }}
              >
                Correct! +10 points
              </div>
            </>
            ...
            <>
              ...
              <div
                style={{ fontWeight: 700, fontSize: "17px", color: "#fbbf24" }}
              >
            ...
            <>
              <div style={{ fontSize: "28px", marginBottom: "6px" }}>😅</div>
              <div
                style={{ fontWeight: 700, fontSize: "17px", color: "#f87171" }}
              >
          ...
          >
            ...
            >
              ...
              <span style={{ fontSize: "15px", color: "var(--text-primary)" }}>
                {missedCount} {missedCount === 1 ? "player" : "players"}
              </span>
            </div>
          </div>
        </div>
        ...
        {myPlayer && (
          <div style={{ textAlign: "center", marginTop: "8px" }}>
            ...
            <span className="score-pill" style={{ fontSize: "16px" }}>
              {myPlayer.score} pts
            </span>
          </div>
        )}
    ...
    <div className="page-container" style={{ padding: "0 0 40px" }}>
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          ...
          {currentPlayer && (
            <span className="score-pill">⭐ {currentPlayer.score}</span>
          )}
        </div>
      </header>
      ...
      >
        ...
        {/* Submit Question */}
        {(gameState?.status !== "voting" || !gameState) && (
          <div
          ...
          >
            ...
            {submitDone ? (
              ...
              >
                ...
                >
                  The host will select it for an upcoming round.
                </div>
              </div>
            ) : (
              <form
              ...
              >
                <p style={{ color: "var(--text-secondary)", fontSize: "14px" }}>
                  Write two truths and one lie about yourself. Mark which one is
                  the lie.
                </p>
                ...
                <button
                  ...
                  className="btn btn-primary"
                  disabled={
                    !s1.trim() || !s2.trim() || !s3.trim() || submitLoading
                  }
