...
export default function AdminPage() {
  ...
  const [questions, setQuestions] = useState<(Question & { players?: { name: string } })[]>([]);
  ...
  useEffect(() => {
    const fetchAll = async () => {
      const [gsRes, plRes, qsRes] = await Promise.all([
        ...
        supabase.from('questions').select('*, players(name)').order('created_at', { ascending: true }),
      ]);
      ...
      if (gs?.current_question_id) {
        const { data: q } = await supabase.from('questions').select('*, players(name)').eq('id', gs.current_question_id).single();
        if (q) setCurrentQuestion(q as Question);
        const { data: v } = await supabase.from('votes').select('*').eq('question_id', gs.current_question_id);
  ...
  useEffect(() => {
    ...
    const questionsSub = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'questions' }, async () => {
        const { data } = await supabase.from('questions').select('*, players(name)').order('created_at', { ascending: true });
        if (data) setQuestions(data as (Question & { players?: { name: string } })[]);
      })
      .subscribe();
    ...
    const votesSub = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'votes' }, async () => {
        ...
        const { data } = await supabase.from('votes').select('*').eq('question_id', gameState.current_question_id);
  ...
  const startRound = async (question: Question) => {
    await withLoading(`start-${question.id}`, async () => {
      await supabase.from('questions').update({ status: 'pending' }).eq('status', 'active');
      await supabase.from('questions').update({ status: 'active' }).eq('id', question.id);
      await updateGameState({ current_question_id: question.id, status: 'waiting' });
  ...
  const revealAnswer = async () => {
    ...
    await withLoading('reveal', async () => {
      ...
      const { data: allVotes } = await supabase.from('votes').select('*').eq('question_id', qid);
      const correctVoters = (allVotes ?? []).filter((v: Vote) => v.selected_index === lieDisplayIndex);

      if (correctVoters.length === 0) {
        // Nobody guessed → question creator gets +10
        const { data: creator } = await supabase.from('players').select('score').eq('id', currentQuestion.player_id).single();
        if (creator) {
          await supabase.from('players').update({ score: creator.score + 10 }).eq('id', currentQuestion.player_id);
        }
      } else {
        // Each correct voter gets +10
        for (const v of correctVoters) {
          const { data: voter } = await supabase.from('players').select('score').eq('id', v.player_id).single();
          if (voter) {
            await supabase.from('players').update({ score: voter.score + 10 }).eq('id', v.player_id);
          }
        }
      }

      await supabase.from('questions').update({ status: 'completed' }).eq('id', qid);
  ...
  return (
    <div className="page-container" style={{ padding: '0 0 60px' }}>
      ...
      {/* Header */}
      <header style={{
        background: 'rgba(13,15,26,0.9)', backdropFilter: 'blur(12px)',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <div>
            <span style={{ fontWeight: 800, fontSize: '15px' }} className="gradient-text">Admin Dashboard</span>
            <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginTop: '1px' }}>Two Truths &amp; a Lie</div>
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          {gameState && (
            <span className={`badge ${gameState.status === 'voting' ? 'badge-green' : gameState.status === 'revealed' ? 'badge-yellow' : 'badge-purple'}`}>
              {gameState.status === 'voting' ? '🗳️ VOTING' : gameState.status === 'revealed' ? '🎉 REVEALED' : '⏳ WAITING'}
            </span>
          )}
          <button className="btn btn-ghost btn-sm" onClick={resetGame} disabled={actionLoading === 'reset'}>
            🔄 Reset Game
          </button>
        </div>
      </header>
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '28px 20px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: '24px', alignItems: 'start' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            ...
            {currentQuestion && (
              <div className={gameState?.status === 'voting' ? 'card-glow' : 'card'} style={{ animation: 'fadeInUp 0.4s ease' }}>
                <div className="section-header">🎯 Active Round</div>

                <div style={{ marginBottom: '16px' }}>
                  <div style={{ color: 'var(--text-secondary)', fontSize: '13px', marginBottom: '8px' }}>
                    Submitted by <strong style={{ color: 'var(--text-primary)' }}>{(currentQuestion as Question & { players?: { name: string } })?.players?.name ?? '?'}</strong>
                  </div>
                  {(() => {
                    const sm = getShuffleMap(currentQuestion);
                    const stmts = [currentQuestion.statement_1, currentQuestion.statement_2, currentQuestion.statement_3];
                    return (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                        {sm.map((origIdx, displayIdx) => {
                          const isLie = origIdx === currentQuestion.lie_index;
                          const voteCount = votes.filter(v => v.selected_index === displayIdx).length;
                          return (
                            <div key={displayIdx} style={{
                              display: 'flex', alignItems: 'center', gap: '12px',
                              padding: '10px 14px', borderRadius: '10px',
                              background: isLie ? 'rgba(239,68,68,0.08)' : 'rgba(255,255,255,0.03)',
                              border: `1px solid ${isLie ? 'rgba(239,68,68,0.25)' : 'var(--border)'}`,
                              fontSize: '14px'
                            }}>
                              <span style={{ color: isLie ? '#f87171' : 'var(--text-muted)', fontSize: '18px' }}>
                                {isLie ? '🚫' : '✅'}
                              </span>
                              <span style={{ flex: 1 }}>{stmts[origIdx]}</span>
                              {isLie && (
                                <span className="badge badge-red" style={{ fontSize: '10px' }}>LIE</span>
                              )}
                ...
                {gameState?.status === 'voting' && (
                  <div style={{
                    ...
                    marginBottom: '16px', fontSize: '14px', color: '#a78bfa'
                  }}>
                    <span>🗳️ Live votes received:</span>
                    <strong style={{ fontSize: '18px' }}>{votes.length}</strong>
                    <span style={{ color: 'var(--text-muted)' }}>/ {players.length}</span>
                  </div>
                )}
                ...
                <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                  {gameState?.status === 'waiting' && (
                    <button className="btn btn-success" onClick={startVoting} disabled={actionLoading === 'start-voting'}>
                      {actionLoading === 'start-voting' ? <><div className="spinner" />Starting...</> : '▶️ Start Voting'}
                    </button>
                  )}
                  {gameState?.status === 'voting' && (
                    <button className="btn btn-warning" onClick={endVoting} disabled={actionLoading === 'end-voting'}>
                      {actionLoading === 'end-voting' ? <><div className="spinner" />Ending...</> : '⏹️ End Voting'}
                    </button>
                  )}
                  {(gameState?.status === 'voting' || gameState?.status === 'waiting') && (
                    <button className="btn btn-primary" onClick={revealAnswer} disabled={!!actionLoading}>
                      {actionLoading === 'reveal' ? <><div className="spinner" />Revealing...</> : '🎉 Reveal Answer'}
                    </button>
                  )}
                </div>
              </div>
            ...
            {/* Pending Questions */}
            <div className="card">
              <div className="section-header">📋 Submitted Questions ({pendingQuestions.length})</div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  {pendingQuestions.map(q => (
                    <div key={q.id} style={{
                      ...
                      borderRadius: '12px', padding: '16px', transition: 'border-color 0.2s'
                    }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px', flexWrap: 'wrap', gap: '8px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                          <span style={{ fontSize: '16px' }}>👤</span>
                          <span style={{ fontWeight: 600, fontSize: '15px' }}>{q.players?.name ?? 'Unknown'}</span>
                        </div>
                        <div style={{ display: 'flex', gap: '8px' }}>
                          ...
                          >
                            {actionLoading === `start-${q.id}` ? <><div className="spinner" />Loading...</> : '🎯 Start Round'}
                          </button>
                          <button
                            ...
                            disabled={!!actionLoading}
                          >
                            {actionLoading === `delete-${q.id}` ? <div className="spinner" /> : '🗑️'}
                          </button>
                        </div>
                      </div>

                      <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                        {[q.statement_1, q.statement_2, q.statement_3].map((stmt, i) => (
                          <div key={i} style={{
                            display: 'flex', alignItems: 'flex-start', gap: '8px',
                            padding: '8px 10px', borderRadius: '8px',
                            background: i === q.lie_index ? 'rgba(239,68,68,0.08)' : 'rgba(255,255,255,0.02)',
                            border: `1px solid ${i === q.lie_index ? 'rgba(239,68,68,0.2)' : 'transparent'}`
                          }}>
                            <span style={{ color: i === q.lie_index ? '#f87171' : 'var(--text-muted)', fontSize: '14px', marginTop: '1px', flexShrink: 0 }}>
                              {i === q.lie_index ? '🚫' : '✅'}
                            </span>
                            <span style={{ fontSize: '14px', flex: 1, color: 'var(--text-primary)' }}>{stmt}</span>
                            {i === q.lie_index && (
                              <span className="badge badge-red" style={{ fontSize: '10px', flexShrink: 0 }}>LIE</span>
                            )}
                          </div>
                        ))}
                      </div>
            ...
            {completedQuestions.length > 0 && (
              <div className="card">
                <div className="section-header">✅ Completed Rounds ({completedQuestions.length})</div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {completedQuestions.map(q => (
                    <div key={q.id} style={{
                      ...
                      <span>✅</span>
                      <span style={{ color: 'var(--text-secondary)' }}>{q.players?.name}</span>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', position: 'sticky', top: '80px' }}>
            ...
            <div className="card">
              ...
              {players.length === 0
                ? <p style={{ color: 'var(--text-muted)', fontSize: '14px', textAlign: 'center', padding: '20px' }}>No scores yet</p>
